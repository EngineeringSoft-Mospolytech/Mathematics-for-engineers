       ...
       // public delegate double Func(double x);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if(rbLagrange.Checked)
                res = Math.Abs(gausMethod.Solve(function, -1, 1, N));
...
namespace GausMethodIntegral
{
    ...
    abstract public class Methods
    {
        ...
        //  Полином Лежандра
        private double Legandr(double x, double[] func)
        {
            double ret = 0;
            for (int i = 0; i < func.Length; i++)
            {
                ret += Math.Pow(x, i) * func[i];
            }
            return ret;
        }

        //  Поиск корней полинома Лежандра
        protected double Gauss_Legandr(int i, int n, double eps = 0.01f, double e = 0.001f)
        {
            int cnt;
            ...
            for (int k = 0; k <= cnt; k++)
            {
                double t = factorial(n) / (factorial(k) * factorial(n - k)) * Math.Pow(-1, k);
                a[2 * (n - k)] = t;
            }
            ...
            //double C = 1 / (Math.Pow(2, n) * factorial(n));
            ...
            double bx = 1 + max / a[mp];
            double ax = -bx;
            int number = 1;
            for (double X = ax; X <= bx; X += e)
            {
                if ((Math.Abs(Legandr(X, a)) <= eps) && (number == i))
                    return X;
                number++;
            }
            return -1;
    ...
    public class GausMethod : Methods
    {
        ...
        public double SolveLagrange(Function f, int n)
        {
            N = n;
            a = -1;
            b = 1;
        ...
        //  Вычисление определённых интегралов квадратурой Гаусса методом Гаусса-Лагранжа
        private double glfn()
        {
            double r = 0;
            double[] x = new double[N];
